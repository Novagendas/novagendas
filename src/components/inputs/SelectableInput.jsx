      ...
      {isOpen ? (
        <div className="selectable-input-trigger open">
          ...
          {search && (
            <button 
              type="button" 
              className="selectable-input-clear" 
              onClick={() => setSearch('')}
        ...
        >
          ...
          <div className={`selectable-input-text ${selectedOpt ? 'selected' : 'placeholder'}`}>
            {selectedOpt
              ? isClientSearch
                ? `${selectedOpt.label} ${selectedOpt.cedula ? `— ${selectedOpt.cedula}` : ''}`
                : selectedOpt.label
              : placeholder}
          </div>
          <svg
        ...
        <ul className="selectable-input-options animate-scale-in" role="listbox">
          {filtered.length > 0 ? (
              ...
              <li key={opt.value} role="listitem">
                ...
                >
                  <span className="selectable-input-option-label">
                    {opt.label}
                    {isClientSearch && opt.cedula && (
                      <span className="selectable-input-option-cedula">
                        {opt.cedula}
                      </span>
                    )}
                  </span>
            ...
            <div className="selectable-input-no-results" role="status" aria-live="polite">
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 0.5, marginBottom: '8px' }}>
                <circle cx="11" cy="11" r="8"></circle>
                <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
              </svg>
